...
export default function UsersTable() {
  ...
  // Открыть модалку добавления очков
  ...
  const handleAddPoints = async () => {
    ...
    if (!pointsToAdd || isNaN(points)) {
      alert("Пожалуйста, введите корректное число");
      return;
    }

    try {
      console.log("Отправка запроса на добавление очков:", {
        user_id: selectedUser.user_id,
        points: points
      });

      await api.post(`/users/${selectedUser.user_id}/add_points/`, {
        points: points
      });

      alert(`Успешно добавлено ${points} очков пользователю ${selectedUser.username}`);
      closePointsModal();
      await load();
    } catch (error: any) {
      console.error("Ошибка добавления очков:", error);
      console.error("Детали ошибки:", error.response?.data);
      alert(`Не удалось добавить очки: ${error.response?.data?.error || error.message}`);
    }
  };
      ...
      {showPointsModal && selectedUser && (
        <ModalBg onClick={closePointsModal}>
          <Modal onClick={(e) => e.stopPropagation()}>
            ...
            <p>
              <strong>Пользователь:</strong> {selectedUser.first_name} (@{selectedUser.username})
            </p>
            <p>
              <strong>Текущие очки:</strong> {selectedUser.points}
            </p>
            ...
            <div>
              <PointsBtn onClick={handleAddPoints}>Добавить</PointsBtn>
